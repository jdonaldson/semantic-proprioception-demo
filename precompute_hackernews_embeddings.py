#!/usr/bin/env python3
"""
Semantic Proprioception - Pre-compute Hacker News post embeddings

Generates embeddings for HN posts using 4 different models.
"""
...
# Model configurations (same as other demos for consistency)
MODELS = [
    {
        "name": "MiniLM-L3",
...
def load_hackernews_data(csv_path):
    ...
    for row in category_counts.to_dicts():
        print(f"    {row['category_name']}: {row['count']}")

    return df


def embed_posts(posts, model_config):
    """Embed HN posts using specified model"""
    print(f"\n{'='*70}")
...
def build_lsh_index(embeddings, model_config):
    ...
    print(f"  ✓ Dense buckets (≥5 posts): {len(dense_buckets)}")

    if histogram:
        max_size = max(histogram.keys())
        print(f"  ✓ Largest bucket: {max_size} posts")
...
def save_model_data(model_config, df, embeddings, index):
    ...
    output_dir = Path("hackernews_demo_data")
...
def main():
    print("="*70)
    print("Semantic Proprioception - Pre-computing Hacker News Embeddings")
    print("="*70)
    print()

    # Configuration
    CSV_PATH = "hackernews_data/hackernews_posts.csv"

    if not Path(CSV_PATH).exists():
        print(f"❌ Dataset not found at {CSV_PATH}")
        print("   Run: python fetch_hackernews_data.py")
        return

    # Load data
    df = load_hackernews_data(CSV_PATH)

    # Combine title and text for embedding
    posts = []
    for row in df.to_dicts():
        # For HN: title is always present, text may be link description
    ...
    for model_config in MODELS:
        # Embed
        embeddings = embed_posts(posts, model_config)
    ...
    print("  Add dataset selector for Twitter / ArXiv / Hacker News")
    print()
